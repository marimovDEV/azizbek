...
import requests
from config import API_BASE_URL, ADMIN_ID
...
async def start(message: Message, state: FSMContext):
    ...
    try:
        requests.post(f"{API_BASE_URL}/users/", json=user_data, timeout=5)
    except Exception as e:
        logging.error(f"User API registration error: {e}")

    # Set commands based on user role
    settings_resp = requests.get(f"{API_BASE_URL}/users/{message.from_user.id}/")
    admin_id = ADMIN_ID
    if settings_resp.status_code == 200:
        user_info = settings_resp.json()
        if user_info.get('user_id') == admin_id:
            commands = [
...
async def enter_phone(message: Message, state: FSMContext):
    ...
    user_id = message.from_user.id
    try:
        requests.patch(f"{API_BASE_URL}/users/{user_id}/", json={"phone": phone}, timeout=5)
    except Exception as e:
        logging.error(f"User phone update API error: {e}")
...
async def confirm_order(callback: CallbackQuery, state: FSMContext):
    ...
    order_data = {
        "user": requests.get(f"{API_BASE_URL}/users/?user_id={callback.from_user.id}").json()[0]["id"],
    ...
    try:
        requests.post(f"{API_BASE_URL}/orders/", json=order_data, timeout=5)
    except Exception as e:
        logging.error(f"Order create API error: {e}")
    await callback.message.answer(
        f"✅ Buyurtma tasdiqlandi!\n\nTez orada siz bilan bog'lanishadi."
    )
    # Admin notification (can be left as is or refactored to use API if needed)
    await state.clear()
    await callback.answer()
...
async def get_direction_kb():
    ...
    if not routes:
        # Default routes if API is empty
        routes = [
            {"name": "Xorazmdan Buxoroga"},
            {"name": "Buxorodan Xorazmga"}
        ]
...
async def get_car_kb():
    ...
    cars = response.json()
    if not cars:
        # Default cars if API is empty
        cars = [
            {"name": "Kaptiva"}, {"name": "Malibu"}, {"name": "Cobalt"},
            {"name": "Gentra"}, {"name": "Largus"}, {"name": "Lasetti"}
        ]
    rows = []
